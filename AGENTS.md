...
- `internal/tools` owns built-ins, schemas, registry/subsets, dispatch recovery, and truncation. `internal/plan` and `internal/todo` own durable planning and checklist state; `internal/handoff`, `delegate`, and `background` hold approval, agent-starting, or persisted runtime state outside `tools` to avoid import cycles.
- `internal/config`, `agentdef`, and `modelcatalog` handle precedence, agent definitions, and normalized models.dev/OpenAI Codex catalog metadata. `internal/hooks`, `session`, `sysprompt`, and `skills` handle lifecycle hooks, durable transcripts/artifacts, prompt composition, and skill disclosure. Session writes should use temp-file then rename. `internal/sessionrec` is the one canonical raw.ndjson recorder shared by the parent UI sink and delegate child sessions; it owns the replay Display-line formatters (the live renderer in `internal/ui` keeps thin wrappers). `internal/runstream` owns the JSON run-stream vocabulary (`-format json` run modes, both directions: NDJSON input decoding and event output) and mirrors the recorded event stream to stdout.
