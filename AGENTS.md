...
- `internal/config`, `agentdef`, and `modelsdev` handle precedence, agent definitions, and catalog metadata. `internal/hooks`, `session`, `sysprompt`, and `skills` handle lifecycle hooks, durable transcripts/artifacts, prompt composition, and skill disclosure. Session writes should use temp-file then rename.
