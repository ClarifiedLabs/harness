...
- `cmd/harness-model-proxy`: provider/model proxy binary (`[flags]` to serve HTTP, `--setup`, `--refresh-models`, and the `auth` subcommand `<login|logout|status>`); thin CLI over `internal/modelproxy`. Owns provider config, API keys, model-catalog metadata, and concrete provider calls so `harness` only needs a proxy URL.
