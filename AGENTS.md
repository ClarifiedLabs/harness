...
- `internal/tools` owns built-ins, schemas, registry/subsets, dispatch recovery, and truncation. `internal/workstate` owns durable structured planning/execution state; `internal/handoff`, `delegate`, and `background` hold approval, agent-starting, or persisted runtime state outside `tools` to avoid import cycles.
