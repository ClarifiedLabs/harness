...
func TestResolveServeLifecycleSettingPrecedence(t *testing.T) {
	configured := server.Duration{Duration: 7 * time.Second, Set: true}
	tests := []struct {
		name      string
		flagValue string
		flagSet   bool
		...
		{name: "default", fallback: 5 * time.Second, want: 5 * time.Second},
		{name: "config", config: configured, fallback: 5 * time.Second, want: 7 * time.Second},
		{name: "environment", envValue: "8s", config: configured, fallback: 5 * time.Second, want: 8 * time.Second},
		{name: "flag", flagValue: "9s", flagSet: true, envValue: "8s", config: configured, fallback: 5 * time.Second, want: 9 * time.Second},
		{name: "explicit zero", flagValue: "0", flagSet: true, envValue: "8s", config: configured, fallback: 5 * time.Second, want: 0},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, err := resolveServeDuration("drain-delay", tc.flagValue, tc.flagSet, tc.envValue, tc.config, tc.fallback)
			if err != nil {
				t.Fatal(err)
			}
			if got != tc.want {
	...
	if _, err := resolveServeDuration("drain-delay", "", true, "", server.Duration{}, time.Second); err == nil {
		t.Fatal("explicit empty duration was accepted")
	}
	if _, err := resolveServeDuration("drain-delay", "", false, "-1s", server.Duration{}, time.Second); err == nil {
		t.Fatal("negative environment duration was accepted")
	}
}

func TestResolveServeInstanceIDPrecedence(t *testing.T) {
	if got := resolveServeInstanceID("", false, "", "pod-config"); got != "pod-config" {
		t.Fatalf("config instance = %q", got)
	}
	if got := resolveServeInstanceID("", false, " pod-env ", "pod-config"); got != "pod-env" {
		t.Fatalf("environment instance = %q", got)
	}
	if got := resolveServeInstanceID(" pod-flag ", true, "pod-env", "pod-config"); got != "pod-flag" {
		t.Fatalf("flag instance = %q", got)
	}
}
