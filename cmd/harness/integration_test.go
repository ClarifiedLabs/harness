...
func TestSmokeCandidateLineagePreservesAcceptedTreeAndEvidence(t *testing.T) {
	bin := buildBinary(t)
	...
	runGit(repo, "init", "-q")
	runGit(repo, "config", "user.email", "integration@example.test")
	runGit(repo, "config", "user.name", "Harness Integration")
	if err := os.WriteFile(filepath.Join(repo, "seed.txt"), []byte("seed\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	runGit(repo, "add", "seed.txt")
	runGit(repo, "commit", "-qm", "test: seed lineage worktree")
	worktree := filepath.Join(t.TempDir(), "worktree")
	runGit(repo, "worktree", "add", "--detach", worktree, "HEAD")
	if err := os.WriteFile(filepath.Join(worktree, "candidate.txt"), []byte("accepted\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(worktree, "evidence.txt"), []byte("verified evidence\n"), 0o600); err != nil {
		t.Fatal(err)
	}

	hookDir := t.TempDir()
	hookCommand := filepath.Join(hookDir, "accept-candidate")
	hookBody := "#!/bin/sh\nprintf '%s' '{\"accepted\":true,\"score\":10,\"score_direction\":\"maximize\",\"candidate\":\"candidate:accepted\",\"remaining_requirements\":0,\"evidence_ref\":\"evidence.txt\"}'\n"
	if err := os.WriteFile(hookCommand, []byte(hookBody), 0o700); err != nil {
		t.Fatal(err)
	}
	hookPath := filepath.Join(hookDir, "hooks.json")
	hookConfig, err := json.Marshal(map[string]any{
		"Stop": []any{map[string]any{
			"hooks": []any{map[string]any{
				"type": "command", "name": "lineage-accept", "command": hookCommand,
			}},
		}},
	})
	if err != nil {
	...
	if err := os.WriteFile(hookPath, hookConfig, 0o600); err != nil {
		t.Fatal(err)
	}

	mock := &recordingMock{scripts: []string{textTurn("candidate complete")}}
	srv := httptest.NewServer(mock)
	defer srv.Close()
	sessionDir := filepath.Join(t.TempDir(), "session")
	cmd, stdout, errBuf, _ := startHarnessInDir(
		t, bin, srv.URL+"/v1", worktree,
		"-hooks", hookPath,
		"-session", sessionDir,
		"-candidate-lineage",
		"-p", "finish the prepared candidate",
	)
	...
	if len(state.Entries) != 1 || state.BestScore == nil || *state.BestScore != 10 || state.BestCandidate != "candidate:accepted" {
		t.Fatalf("lineage state = %+v", state)
	}
	evidence, err := os.ReadFile(filepath.Join(sessionDir, filepath.FromSlash(state.Entries[0].EvidenceArtifact)))
	if err != nil || string(evidence) != "verified evidence\n" {
		t.Fatalf("preserved evidence = %q, %v", evidence, err)
	}
	if got := strings.TrimSpace(runGitOutput(t, repo, "show", state.BestTree+":candidate.txt")); got != "accepted" {
		t.Fatalf("accepted tree candidate = %q", got)
	}
	...
	for _, line := range strings.Split(strings.TrimSpace(string(raw)), "\n") {
		...
		if event.Type == session.EventLineageAdvance {
			advances++
			if event.LineageAdvance == nil || event.LineageAdvance.Sequence != 1 || event.Display != "" {
				t.Fatalf("lineage event = %+v", event)
			}
		}
	}
	if advances != 1 {
		t.Fatalf("lineage advance events = %d", advances)
	}
}

func TestSmokeCandidateLineageInteractiveExportAndRestore(t *testing.T) {
	bin := buildBinary(t)
	repo := filepath.Join(t.TempDir(), "repo")
	if err := os.MkdirAll(repo, 0o755); err != nil {
		t.Fatal(err)
	...
	runGit("init", "-q")
	runGit("config", "user.email", "integration@example.test")
	runGit("config", "user.name", "Harness Integration")
	if err := os.WriteFile(filepath.Join(repo, "candidate.txt"), []byte("base\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(repo, "evidence.txt"), []byte("verified evidence\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	runGit("add", "-A")
	runGit("commit", "-qm", "test: seed interactive lineage")
	beforeHead := strings.TrimSpace(runGitOutput(t, repo, "rev-parse", "HEAD"))
	indexPath := strings.TrimSpace(runGitOutput(t, repo, "rev-parse", "--git-path", "index"))
	...
	writeArgs, _ := json.Marshal(map[string]any{"path": "candidate.txt", "content": "regressed\n"})
	mock := &recordingMock{scripts: []string{
		textTurn("candidate complete"),
		namedToolCallTurn("call_regress", "write", string(writeArgs)),
		textTurn("regression complete"),
	}}
	srv := httptest.NewServer(mock)
	defer srv.Close()
	sessionDir := filepath.Join(t.TempDir(), "session")
	exportDir := filepath.Join(t.TempDir(), "accepted-export")
	input := strings.Join([]string{
		"finish the prepared candidate",
		"make a later change",
		"/evidence --kind evaluator --limit 5",
		"/lineage",
		"/lineage export best " + exportDir,
		"/lineage restore best --force",
		"/exit",
		"",
	}, "\n")
	cmd, stdout, errBuf, _ := startInteractiveHarnessInDir(
		t, bin, srv.URL+"/v1", repo, input,
		"-hooks", hookPath,
		"-session", sessionDir,
		"-candidate-lineage",
		"-no-steer",
	...
	for _, want := range []string{"evidence catalog: 2 of 2 matching records", "evidence.txt", "1 accepted checkpoint", "lineage exported #1", "lineage restored #1", "recovery patch"} {
		if !strings.Contains(errBuf.String(), want) {
			t.Errorf("stderr missing %q:\n%s", want, errBuf.String())
		}
	}
	if got, err := os.ReadFile(filepath.Join(repo, "candidate.txt")); err != nil || string(got) != "base\n" {
		// The first model response did not mutate the prepared base candidate, so
		// the accepted checkpoint is the initial base content.
		t.Fatalf("restored candidate = %q, %v", got, err)
	}
	if got, err := os.ReadFile(filepath.Join(exportDir, "candidate.txt")); err != nil || string(got) != "base\n" {
	...
	if len(state.Entries) != 1 || state.Entries[0].Score != 10 {
		t.Fatalf("lineage state = %+v", state)
	}
	backups, err := os.ReadDir(filepath.Join(sessionDir, "lineage", "restore-backups"))
	if err != nil || len(backups) != 1 {
		t.Fatalf("restore backups = %v, %v", backups, err)
	}
	...
	if !bytes.Equal(afterIndex, beforeIndex) {
		t.Fatal("interactive lineage changed the real Git index")
	}
	if afterHead := strings.TrimSpace(runGitOutput(t, repo, "rev-parse", "HEAD")); afterHead != beforeHead {
		t.Fatalf("HEAD after interactive lineage = %s, want %s", afterHead, beforeHead)
	}
}
