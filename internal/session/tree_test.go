...
func TestCompactionMaterializesRewrittenKeptSuffix(t *testing.T) {
	...
	checkpoint.Compaction = &llm.CompactionMetadata{Summary: "summary", SummarySource: "deterministic", FallbackReason: "timeout", ReadFilesOmitted: 7}
	...
	if got[0].Compaction == nil || got[0].Compaction.Focus != "focus" || !slices.Equal(got[0].Compaction.ReadFiles, []string{"read.go"}) || got[0].Compaction.ReadFilesOmitted != 7 {
