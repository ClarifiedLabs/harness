...
const (
	...
	defaultSubscriptionPollInterval = "5m"
...
var catalog = configmeta.MustCatalog(
	...
	configmeta.Parameter{Key: "subscription_poll_interval", Type: "duration", Flags: []string{"subscription-poll-interval"}, Environment: []string{"HARNESS_MODEL_PROXY_SUBSCRIPTION_POLL_INTERVAL"}, JSONPath: "subscription_poll_interval", Default: literalDefault(defaultSubscriptionPollInterval, defaultSubscriptionPollInterval), Description: "Subscription quota metrics refresh interval (minimum 1m); zero or disabled metrics disables background polling."},
