...
func TestBuildCatalogShape(t *testing.T) {
	...
	if !strings.Contains(out, "If a later turn needs the skill again, re-read its `SKILL.md`.") {
		t.Errorf("catalog should tell the model to re-read skills on later turns: %q", out)
	}
}
...
func TestInjectionBlock(t *testing.T) {
	const want = "<skill>\n<name>pdf</name>\n<path>/skills/pdf/SKILL.md</path>\n\nfirst\nsecond\n</skill>"
	for _, body := range []string{"first\nsecond", "first\nsecond\n", "first\nsecond\n\n"} {
		if got := InjectionBlock("pdf", "/skills/pdf/SKILL.md", body); got != want {
			t.Fatalf("InjectionBlock body %q =\n%s\nwant:\n%s", body, got, want)
		}
	}
}
